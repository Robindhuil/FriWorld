...
**https://github.com/users/Robindhuil/projects/2**. Tento repozitár je `Projekt: FriWorld`.

Ovláda sa cez `~/.claude/plan.py` (na Windows `C:/Users/robob/.claude/plan.py`), pravidlá sú
v `~/.claude/planning-board.md`.

```bash
python ~/.claude/plan.py list --projekt FriWorld
python ~/.claude/plan.py add "Titulok" --projekt FriWorld --typ fix --velkost S \
  --zdroj "docs/…md" --telo "Prečo to tu je."
...
o tom jedným riadkom. Naopak to, čo sa v tej istej session spravilo, na board nepatrí — to je
commit a riadok v `CHANGELOG.md`. Dôvody a čísla zostávajú v `docs/`; karta na ne odkazuje
poľom `Zdroj`.
